...
Output binary: `bin/Debug/net8.0-windows/AddToAutorun.exe`

## Project Overview

**AddToAutorun** is a Windows Forms app (.NET 8, `net8.0-windows`) that manages Windows startup registry entries under `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` and `HKLM\...`.
...
2. **Silent/CLI mode** (with args): `AddToAutorun.exe <filePath> [/hklm]` — registers without showing UI; silently exits if already registered or on error.
...
| `MainForm.cs` / `MainForm.Designer.cs` | Main UI: drag-drop zone, file card, HKCU/HKLM toggle buttons, Add button, notification bar, "Manage" button, SendTo registration toggle |
| `AutorunListForm.cs` / `AutorunListForm.Designer.cs` | List of all current autorun entries from both hives with per-entry delete |
| `Program.cs` | Entry point — routes to silent mode or GUI mode |
